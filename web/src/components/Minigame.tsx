  ...
  return (
    <Box
      sx={{
        ...
        border: `var(--hairline) solid ${state === 'active' ? colors.accent : 'rgba(255, 255, 255, 0.4)'}`,
        borderRadius: '0.15rem',
        backgroundColor: state === 'done' ? colors.accent : 'rgba(0, 0, 0, 0.6)',
        boxShadow: state === 'active' ? `inset 0 0 1.2vh ${colors.accentGlow}` : 'none'
      }}
    >
      <Typography
        sx={{
    ...
    <Box sx={panelSx}>
      ...
      <Box component="svg" viewBox="0 0 100 100" sx={{ width: '7rem', height: '7rem' }}>
        ...
        <g transform={`rotate(${angle} 50 50)`}>
          <line x1="50" y1="13" x2="50" y2="1" stroke={colors.accent} strokeWidth="2.6" />
        </g>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
        <KeyBadge label={'⏎'} state="active" size="1.1rem" />
...
function Tension({ data, onDone }: MinigameProps) {
  ...
  return (
    <Box sx={panelSx}>
      ...
      >
        <Box sx={{ height: '100%', width: `${progress}%`, background: colors.accent }} />
      </Box>
      <Box
        sx={{
          width: '14rem',
...
function Mash({ data, onDone }: MinigameProps) {
  ...
  return (
    <Box sx={panelSx}>
      ...
      >
        <Box sx={{ height: '100%', width: `${fill}%`, background: colors.accent }} />
      </Box>
      <Box
        sx={{
          width: '14rem',
