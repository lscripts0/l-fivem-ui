...
const typeColor: Record<NotifyType, string> = {
  info: colors.accent,
...
function Toast({ toast, onDone, slide }: { toast: ToastItem; onDone: (id: number) => void; slide: string }) {
  ...
  const Icon = icons[type] ?? InfoOutlined
  const tc = typeColor[type] ?? colors.accent
  ...
  return (
    ...
    >
      ...
      >
        <Icon sx={{ fontSize: '0.9rem', color: tc, filter: `drop-shadow(0 0 0.35rem ${tc})` }} />
      </Box>
      <Box
        sx={{
          position: 'relative',
