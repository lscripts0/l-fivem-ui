local function notify(playerId, message, msgType, duration, title)
    ...
    TriggerClientEvent('l-fivem-ui:notify', playerId, message, msgType, duration, title)
end
...
local function countdown(playerId, seconds, endText)
    if not playerId then return end
    TriggerClientEvent('l-fivem-ui:countdown', playerId, seconds, endText)
end

local function cancelCountdown(playerId)
    if not playerId then return end
    TriggerClientEvent('l-fivem-ui:cancelCountdown', playerId)
end

local function showObjectives(playerId, data)
    if not playerId then return end
    TriggerClientEvent('l-fivem-ui:objectives', playerId, data)
end

local function setObjective(playerId, id, done)
    if not playerId then return end
    TriggerClientEvent('l-fivem-ui:setObjective', playerId, id, done)
end

local function hideObjectives(playerId)
    if not playerId then return end
    TriggerClientEvent('l-fivem-ui:hideObjectives', playerId)
end
