...
local function startKeyboardThread(menu)
    CreateThread(function()
        ...
        while currentMenu == menu do
            if pressedWithRepeat(188, holdUntil) then sendKey('up') end
            if pressedWithRepeat(187, holdUntil) then sendKey('down') end
            if pressedWithRepeat(189, holdUntil) then sendKey('left') end
            if pressedWithRepeat(190, holdUntil) then sendKey('right') end
            if IsControlJustPressed(0, 201) then sendKey('select') end
        ...
        while holdTextUIOpen and holdToken == token do
            ...
            if progress >= 1.0 then
                ...
                if onComplete then
                    ...
                    if not ok then
                        print(('l-fivem-ui: hold textui onComplete failed: %s'):format(tostring(err)))
                    end
                end
                break
            end
...
RegisterNUICallback('dialog:result', function(data, cb)
    ...
    elseif dialog.onSubmit then
        ...
        if not ok then
            print(('l-fivem-ui: dialog onSubmit failed: %s'):format(tostring(err)))
        end
    end
end)
...
RegisterNetEvent('l-fivem-ui:notify', function(message, msgType, duration, title)
...
RegisterNetEvent('l-fivem-ui:countdown', function(seconds, endText)
    startCountdown(seconds, endText)
end)

RegisterNetEvent('l-fivem-ui:cancelCountdown', function()
    cancelCountdown()
end)

RegisterNetEvent('l-fivem-ui:objectives', function(data)
    showObjectives(data)
end)

RegisterNetEvent('l-fivem-ui:setObjective', function(id, done)
    setObjective(id, done)
end)

RegisterNetEvent('l-fivem-ui:hideObjectives', function()
    hideObjectives()
end)
